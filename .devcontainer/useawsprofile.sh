...
useawsprofile(){
  ...
  if [ -z "${_UAP_CONFIG_MFA_SERIAL}" ]; then
    echo "エラー：Configに mfa_serial がありません。"
    set +uC
    return 1
  fi

  if [ -z "${_UAP_CONFIG_TARGET_ROLE_ARN}" ]; then
    echo "エラー：Configに Assume Role 先の role_arn がありません。"
    set +uC
    return 1
  fi
  ...
  if [ -z "${_UAP_CONFIG_REGION}" ]; then
    echo "リージョン未指定なので ap-northeast-1 を使用します。"
  ...
  _UAP_ASSUME_ROLE_OUTPUT=$(aws sts assume-role \
    ...
    --role-session-name ${_UAP_DATE}-session \
